...
function upload_components() {
	...
	if [ "${CONFIG_FS_ROMFS}" == "y" ]; then romfs=1; else romfs=0; fi
	echo "set romfs_partition_enable ${romfs}" > ${ROMFS_FLASHING_CFG}

	pushd ${OPENOCD_DIR_PATH}
	"${OPENOCD}" -f "${CFG_FILE}" "-c ${commands} exit"
	popd >/dev/null
	rm "${ROMFS_FLASHING_CFG:?}" -f
}
